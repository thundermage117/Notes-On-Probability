...
Corollary: $F_X(.)$ is left continuous if and only if $P(X=x)=0 \; \forall \; x \in \mathbb{R} $. From lemma,

$$ P(X=x)= F_X(x)- \lim_{\epsilon \to 0^+ }F_X(x-\epsilon)$$

A function is left continuous if

$$  \lim_{\epsilon \to 0^+}F_X(x - \epsilon)= F_X(x) \quad \forall$$
%incomplete
...
If $F_X(.)$ is differentiable, given that it is continuous we can find a
%incomplete

$$ f_X(x)= \frac{dF_X(x)}{dx}$$

$f_X(x)$ is called the probability distribution function (pdf).
%incomplete
...
$$ P(x \leq X \leq x + \Delta x) \approx$$
%incomplete

\subsubsection{When is a function a valid probability distribution function?}
Two properties:
\begin{itemize}
    \item $f_X(x) \geq 0$
...
The probability distribution function itself doesn't indicate any probability. Only the area under it's curve indicates probability.
